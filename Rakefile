...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/jrg-project-templates/esign/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("esign.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in esign.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "esign.gemspec", /ruby_version = .*">= (.*)"/ => lowest
